...
const ClientTestimonialsSection = () => {
  return (
    <section
      id="testimonials"
      className="bg-gray-50 py-20 px-4 sm:px-8"
    ...
    >
      <div className="max-w-7xl mx-auto">
        ...
        <h2 className="text-4xl sm:text-5xl font-extrabold text-center text-gray-900 mb-4">
          Our Top Clients <span className="text-yellow-500">Speak For Us</span>
        </h2>

        <p className="text-lg text-gray-600 text-center mb-16 max-w-2xl mx-auto">
          Hear from the amazing individuals and businesses we&apos;ve had the
          pleasure of working with. Their success is our success.
        </p>
        ...
        <div className="grid gap-10 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
          {testimonials.map((testimonial, index) => (
            <div
              key={index}
              className="bg-white border border-gray-200 rounded-3xl p-8 shadow-lg transition-all duration-300 transform hover:-translate-y-2 hover:shadow-2xl flex flex-col justify-between"
            >
              {/* Client Image */}
              <img
                src={testimonial.image}
                alt={`Profile picture of ${testimonial.name}`}
                className="w-20 h-20 rounded-full mx-auto mb-6 object-cover border-4 border-yellow-400 shadow-md"
              />

              {/* Quote */}
              <div className="relative">
                <svg
                  className="absolute -top-6 -left-2 w-10 h-10 text-yellow-300 opacity-40"
                  fill="currentColor"
                  viewBox="0 0 24 24"
                >
                  <path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5a5.4 5.4 0 015.4-5.4c1.87 0 3.6 0.95 4.6 2.5a5.4 5.4 0 014.6-2.5c2.97 0 5.4 2.43 5.4 5.4 0 3.78-3.4 6.86-8.55 11.54L12 21.35z" />
